...
export const CompoundTodo = ({ children }) => {
  const [listTodos, setListTodos] = useState({});

  const addTodo = (newTodo) => {
    setListTodos({
      ...listTodos,
      [newTodo]: { name: newTodo, isDone: false }
    });
  };
  ...
  const getTodoValues = () => {
    ...
    return Object.values(listTodos);
  };

  // const todoListValues = useMemo(() => getTodoValues());
